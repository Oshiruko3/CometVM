...
constexpr int SP_REG = 4;          // GR4 = スタックポインタ
constexpr uint16_t COMET_MEM_SIZE = 0x10000;  // 65536語（COMET IIは語アドレス空間）
...
enum Opcode : uint8_t {
    // ロード・ストア系
    OP_LD  = 0x01,   // LD  gr, adr[,xr]  - ロード
    OP_LAD = 0x02,   // LAD gr, adr[,xr]  - ロードアドレス
    OP_ST  = 0x03,   // ST  gr, adr[,xr]  - ストア
    OP_LEA = 0x04,   // LEA gr, adr[,xr]  - GRにadr+xrの内容を加算（ループカウンタの定番）
    // 算術演算系
    OP_ADDA = 0x10,  // ADDA gr, adr[,xr] - 算術加算
    OP_ADDL = 0x11,  // ADDL gr, adr[,xr] - 論理加算
    OP_SUBA = 0x12,  // SUBA gr, adr[,xr] - 算術減算
    OP_SUBL = 0x13,  // SUBL gr, adr[,xr] - 論理減算
    // 論理演算系
    OP_AND = 0x20,   // AND gr, adr[,xr]  - 論理積
    OP_OR  = 0x21,   // OR  gr, adr[,xr]  - 論理和
    OP_XOR = 0x22,   // XOR gr, adr[,xr]  - 排他的論理和
    // 比較・シフト系
    OP_CPA = 0x30,   // CPA gr, adr[,xr]  - 算術比較
    OP_SLL = 0x40,   // SLL gr, n         - 論理左シフト
    OP_SRL = 0x41,   // SRL gr, n         - 論理右シフト
    // 分岐・制御系
    OP_JMP = 0x50,   // JMP adr           - 無条件分岐
    OP_JPZ = 0x51,   // JPZ adr           - FR.Z=1で分岐
    OP_JNZ = 0x52,   // JNZ adr           - FR.Z=0で分岐
    OP_JMI = 0x53,   // JMI adr           - FR.S=1で分岐
    OP_JPL = 0x54,   // JPL adr           - FR.S=0で分岐
...
// COMETの状態
struct Comet {
    int16_t GR[NUM_GR] = {};    // 汎用レジスタ（GR4=SP）
    ...
    // 段階2-3のUI層でTFT表示に繋ぐために追加（8/12・design_notes.md参照）
    void (*out_func)(char) = nullptr;

    // 入力コールバック（IN命令の入力元・nullptrならダミー動作）
    // bufに文字列を取得して長さを返す（0=入力なし）
    // Cardputerのキーボード入力に繋ぐために追加（8/12・design_notes.md参照）
